---
title: "CTF Android Mobile - Config Editor"
date: 2024-03-23
categories:
  - blog
---
...
When you click "Load," the file manager will open, displaying the contents of the SD card. Here, you'll see a file named "example.yaml" ready for you to select and load into the application.
...
> SnakeYaml's Constructor() class does not restrict types which can be instantiated during
> deserialization. Deserializing yaml content provided by an attacker can lead to remote code
> execution. We recommend using SnakeYaml's SafeConsturctor when parsing untrusted content to
> restrict deserialization. We recommend upgrading to version 2.0.
...
+ [https://snyk.io/blog/snakeyaml-unsafe-deserialization-vulnerability/](https://snyk.io/blog/snakeyaml-unsafe-deserialization-vulnerability/){:target="_blank"}
+ [https://nvd.nist.gov/vuln/detail/CVE-2022-1471](https://nvd.nist.gov/vuln/detail/CVE-2022-1471){:target="_blank"}
...
<!-- <p align="center">
<img src="/assets/images/ctf-config-editor/pwn.png" width="500"/>
</p> -->

![image info](/assets/images/ctf-config-editor/pwn.png)

## Shell is the way

Running commands is nice, but what about a shell? <3  
Initially, I tried to execute the classic reverse shells that are publicly available, such as those found on [revshell](https://www.revshells.com/){:target="_blank"} or by uploading a compiled one in `C` (you can find an example in the current folder as `shell.c`).

While exploring various possibilities, I came across this methodology on [malacupa.com](https://malacupa.com/2018/10/25/android-command-line-reverse-shell){:target="_blank"} which utilizes compiling Java code to be executed by ART/Dalvik on Android’s command line.
...
![Alt Text](/assets/images/ctf-config-editor/exploit.gif)

## References

+ [https://snyk.io/blog/snakeyaml-unsafe-deserialization-vulnerability/](https://snyk.io/blog/snakeyaml-unsafe-deserialization-vulnerability/){:target="_blank"}
+ [https://nvd.nist.gov/vuln/detail/CVE-2022-1471](https://nvd.nist.gov/vuln/detail/CVE-2022-1471){:target="_blank"}
+ [android-command-line-reverse-shell](https://malacupa.com/2018/10/25/android-command-line-reverse-shell){:target="_blank"}
